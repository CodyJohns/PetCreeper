...
public class PetEntityListener implements Listener {
    ...
    public void onPetInventoryClick(InventoryClickEvent event) {
    	...
    	if(event.getInventory().getType().equals(InventoryType.CHEST)){
    		...
    		ArrayList<Pet> pets = plugin.getPetsOf(player);
    		for(Pet pet : pets){
    			if(pet.petName.equalsIgnoreCase(event.getInventory().getTitle())){
    ...
    public void onProjectileLaunchEvent(ProjectileLaunchEvent event) {
        ...
        if(this.plugin.isPet(e)){
        	if (p instanceof Fireball) {
        	...
        	else if (p instanceof ThrownPotion) {
                event.setCancelled(true);
                p.remove();
            }
        		...
        		if(p.getShooter() instanceof Skeleton){
        			event.setCancelled(true);
        			p.remove();
        			((Skeleton)p.getShooter()).getEquipment().clear();
        		}
            }
        	else if (this.plugin.isPet(p.getShooter())) {
        		event.setCancelled(true);
        		p.remove();
        	}
        }
